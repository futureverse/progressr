...
- Buffering and flushing of standard output (stdout) would cause a
  ...
  until the last newline, keeping the remaining output buffered until it
  is safe to flush.
...
- Add [`length()`](https://rdrr.io/r/base/length.html) for `progressor`
  objects, which enables using
  [`seq_along()`](https://rdrr.io/r/base/seq.html) and
  [`seq()`](https://rdrr.io/r/base/seq.html) on progressors.

### Beta Features

- Add experimental support for `progressor(..., finalize = TRUE)`, which
  will cause the progressor to signal a progression ‘shutdown’ condition
  when the progressor is garbage collected. This can serve as a backstop
  in yet-to-be-observed cases where a progressor did not shut down
  properly leaving for instance active sinks behind. For now,
  `finalize = FALSE` is the default.
...
- It was not possible to create more than one progressor in a
  ...
  call, resulting in additional progressors being ignored. Also,
  warnings on “with_progress() received a progression ‘initiate’
  request” were produced with **progressr** 0.17.0 (2025-10-15).
...
- Now also
  [`with_progress()`](https://progressr.futureverse.org/reference/with_progress.md)
  warns about receiving progress updates after the progressor has
  finished. Previously, only the global ‘progression’ handler would do
  this.
...
- Using
  [`with_progress()`](https://progressr.futureverse.org/reference/with_progress.md)
  together with `handlers(global = TRUE)` would produce a warning on
  “Received a progression ‘update’ request (…) but is not listening to
  this progressor”.
...
- Add
  ...
  for slowing down a full progress run a certain amount of time (in
  seconds).
...
- RStudio v2025.05.0 introduced new default handlers for messages and
  ...
  RStudio v2025.05.1. If you run v2025.05.0, **progressr** implements a
  workaround.
...
- Package failed to load in the Positron environment due to an “object
  ‘RStudio.Version’ of mode ‘function’ was not found” error.
...
- Add
  ...
  for reporting on progress via the Ntfy.sh Messaging Service using the
  **ntfy** package.

### Bug Fixes

- [`withProgressShiny()`](https://progressr.futureverse.org/reference/withProgressShiny.md)
  could produce an \`if (config\$max_steps ==

  0.  : … argument is of length zero\` error.

- `handlers(new_handlers)` would return `NULL`, instead of
  [`list()`](https://rdrr.io/r/base/list.html), if there were no prior
  handlers set.

- `handler_cli(..., format_done = "...", clear = TRUE)` would not render
  the `format_done` message, if set.
...
- Progress reporting may be terminated by an interrupt (e.g. user
  ...
  run-time error. When this happens, the most recent progress update is
  preserved (e.g. a progress bar in the terminal remains), and an
...
- Now the ‘shiny’ and ‘filesize’ handlers are enabled by default.
  Previously, they were only enabled in interactive mode, but as these
  are frequently used also in non-interactive mode, it’s less confusing
  if they’re always enabled, e.g. Shiny applications are often run via a
...
- Add
  [`handler_rpushbullet()`](https://progressr.futureverse.org/reference/handler_rpushbullet.md)
  for reporting on progress via the Pushbullet Messaging Service using
  the **RPushbullet** package.
...
- Now progress updates of type “finish” supports also updating the
  ...
  whereas previously you had to do `p(amount = 1.0)` and then
  `p(type = "finish")` resulting in two progress conditions being
  signaled.
...
- The ‘cli’ handler did not handle zero-length progressors resulting in
  `Error in rep(chr_complete, complete_len) : invalid 'times' argument`
  when the progressor completed.

- The ‘cli’ handler did not work when the **cli** package was configured
  to report on progress via **progressr**, i.e. when setting
...
- Add
  [`handler_cli()`](https://progressr.futureverse.org/reference/handler_cli.md)
  for rendering progress updates via the **cli** package and its
  [`cli::cli_progress_bar()`](https://cli.r-lib.org/reference/cli_progress_bar.html).

- Now
  [`handler_progress()`](https://progressr.futureverse.org/reference/handler_progress.md)
  creates a **progress** progress bar that is always rendered by forcing
...
- [`handler_txtprogressbar()`](https://progressr.futureverse.org/reference/handler_txtprogressbar.md)
  gained support for ANSI-colored `char` ASCII and Unicode symbols.

### Miscellaneous

- Now
  [`with_progress()`](https://progressr.futureverse.org/reference/with_progress.md)
  asserts that the number of active “output” sinks is the same on exit
  as on enter, and that the last one closed is the one that was created.
...
- Using
  [`with_progress()`](https://progressr.futureverse.org/reference/with_progress.md)
  while the global progress handler was enabled could result in errors
  for the **cli** handler, and possibly for other progression handlers
  developed in the future. Because of this,
  [`with_progress()`](https://progressr.futureverse.org/reference/with_progress.md)
  and
  [`without_progress()`](https://progressr.futureverse.org/reference/with_progress.md)
  now disables the global progress handler temporarily while running.

- The `pbmclapply()` handler went from 0 to 100% in one step, because we
  forgot to set the `max`:imum value.
...
- When using a ‘winprogressbar’ or a ‘tkprogressbar’ handler,
  ...
  updated and from what type of progression message is configured via
  the new `inputs` argument. For example,
  `inputs = list(title = "sticky_message", label = "message")` causes
  progression messages to update the `label` component and sticky ones
  to update both. For backward compatible reasons, the default is
...
- Warnings on stray `progression` conditions could appear with an empty
  message.
...
- Now interrupts are detected, which triggers the progress handlers to
  terminate nicely, e.g. a progress bar in the terminal will stay as-is
...
- Function `progress()` is defunct in order to re-use it for other
  purpose. It is unlikely that anyone really used this function, but if
  you did, then use `cond <- progression()` to create a `progression`
...
- [`progressor()`](https://progressr.futureverse.org/reference/progressor.md)
  no longer records the call stack for progressions by default, because
  that significantly increases the size of these condition objects,
  e.g. instead of being 5 kB it may be 500 kB. If a large number of
  progress updates are signaled and collected, as done, for instance, by
  futures, then the memory consumption on the collecting end could
  become very large. The large sizes would also have a negative impact
  on the performance in parallelization with futures because of the
  extra overhead of transferring these extra large conditions from the
  parallel workers back to the main R session. These issues has been
  there since **progressr** 0.7.0 (December 2020). To revert to the
  previous behavior, use `progressor(..., trace = TRUE)`.

### New Features

- [`progressor()`](https://progressr.futureverse.org/reference/progressor.md)
  gained argument `trace` to control whether or not the call stack
...
- Registered progression handlers would report on progress also when in
  ...
  work when using
  [`parallel::mclapply()`](https://rdrr.io/r/parallel/mclapply.html),
  which is not true. Note however, that it does indeed work when using
  the future ‘multicore’ backend, which uses forks.
...
- Creating a new
  [`progressor()`](https://progressr.futureverse.org/reference/progressor.md)
  will now automatically finish an existing progressor, if one was
  previously created in the same environment. The previous behavior was
  to give an error (see below bug fix).
...
- [`withProgressShiny()`](https://progressr.futureverse.org/reference/withProgressShiny.md)
  gained argument `inputs`, which can be used to control whether or not
  Shiny progress components `message` and `detail` should be updated
  based on the progression message, e.g.
  `inputs = list(message = "sticky_message", detail = "message")` will
  cause progression messages to update the `detail` component and sticky
  ones to update both.

- Now supporting zero-length progressors,
  e.g. `p <- progressor(along = x)` where `length(x) == 0`.

- Add `handlers("rstudio")` to report on progress in the RStudio Console
  via the RStudio Job interface.
...
- As an alternative to specifying the relative amount of progress, say,
  ...
  concurrently, because we cannot guarantee the order these progressions
  arrive.

### Bug Fixes

- In **progressr** 0.7.0, any attempt to use more than one progressor
  inside a function or a [`local()`](https://rdrr.io/r/base/eval.html)
  call would result in: “Error in assign(”…progressor”, value = fcn,
  envir = envir) : cannot change value of locked binding for
  …progressor.”
...
- The user can now use `handlers(global = TRUE)` to enable progress
  reports everywhere without having to use
  [`with_progress()`](https://progressr.futureverse.org/reference/with_progress.md).
  This only works in R (\>= 4.0.0) because it requires global calling
  handlers.

- [`with_progress()`](https://progressr.futureverse.org/reference/with_progress.md)
  now reports on progress from multiple consecutive progressors,
...
- A progressor must not be created in the global environment unless
  ...
  or
  [`without_progress()`](https://progressr.futureverse.org/reference/with_progress.md)
  call. Ideally, a progressor is created within a function or a
  [`local()`](https://rdrr.io/r/base/eval.html) environment.
...
- [`progressor()`](https://progressr.futureverse.org/reference/progressor.md)
  gained argument `enable` to control whether or not the progressor
  signals `progression` conditions. It defaults to option
  ...
  provide a `progress = TRUE/FALSE` argument in their functions to
  migrate to the **progressr** package without having to change their
  existing API, e.g. the setup becomes
  `p <- progressor(along = x, enabled = progress)`. The
  [`p()`](https://rstudio.github.io/htmltools/reference/builder.html)
  function created by `p <- progressor(..., enable = FALSE)` is an empty
...
- The progression message can now be created dynamically based on the
  information in the `progression` condition. Specifically, if `message`
  is a function, then that function will called with the `progression`
  condition as the first argument. This function should return a
  character string. Importantly, it is only when the progression handler
  receives the progression update and calls `conditionMessage(p)` on it
  that this function is called.

- [`progressor()`](https://progressr.futureverse.org/reference/progressor.md)
  gained argument `message` to set the default message of all
  progression updates, unless otherwise specified.

- [`progressor()`](https://progressr.futureverse.org/reference/progressor.md)
  gained argument `on_exit = TRUE`.

- Now the `progress` handler shows also a spinner by default.

- Add the ‘pbcol’ handler, which renders the progress as a colored
  progress bar in the terminal with any messages written in the front.
...
- Now
  [`with_progress()`](https://progressr.futureverse.org/reference/with_progress.md)
  makes sure that any output produced while reporting on progress will
  not interfere with the progress output and vice versa, which otherwise
  is a common problem with progress frameworks that output to the
  terminal, e.g. progress-bar output is interweaved with printed
  objects. In contrast, when using **progressr** we can use
  [`message()`](https://rdrr.io/r/base/message.html) and
  [`print()`](https://rdrr.io/r/base/print.html) as usual regardless of
  progress being reported or not.

### New Features

- Signaling `progress(msg, class = "sticky")` will cause the message to
  be sticky, e.g. for progress bars outputting to the terminal, the
  message will be “pushed” above the progress bar.

- [`with_progress()`](https://progressr.futureverse.org/reference/with_progress.md)
  gained argument `delay_terminal` whose default will be automatically
  inferred from inspecting the currently set handlers and whether they
  output to the terminal or not.

- Arguments `delay_stdout` and `delay_conditions` for
  [`with_progress()`](https://progressr.futureverse.org/reference/with_progress.md)
  is now agile to the effective value of the `delay_terminal` argument.

- Now handler_nnn() functions pass additional arguments in `...` to the
  underlying progress-handler backend,
...
- Add environment variables `R_PROGRESSR_CLEAR`, `R_PROGRESSR_ENABLE`,
  `R_PROGRESSR_ENABLE_AFTER`, `R_PROGRESSR_TIMES`, and
  `R_PROGRESSR_INTERVAL` for controlling the default value of the
  corresponding `progressr.*` options.
...
- The `progress` handler, which uses
  [`progress::progress_bar()`](http://r-lib.github.io/progress/reference/progress_bar.md),
  did not support colorization of the `format` string when done by the
  `crayon` package.

- [`handlers()`](https://progressr.futureverse.org/reference/handlers.md)
  did not return invisible (as documented).
...
- [`progressor()`](https://progressr.futureverse.org/reference/progressor.md)
  gained arguments `offset` and `scale`, and `transform`.

- [`handlers()`](https://progressr.futureverse.org/reference/handlers.md)
  gained argument `append` to make it easier to append handlers.
...
- [`handlers()`](https://progressr.futureverse.org/reference/handlers.md)
  did not return a list if the ‘default’ handler was returned.

## Version 0.2.0

### Significant Changes

- Renamed `withProgress2()` to
  [`withProgressShiny()`](https://progressr.futureverse.org/reference/withProgressShiny.md).

### New Features

- [`handlers()`](https://progressr.futureverse.org/reference/handlers.md)
  gained argument `default` specifying a progression handler to be
...
- Add `withProgress2()`, which is a plug-in backward compatibility
  ...
  wrapped in
  [`progressr::with_progress()`](https://progressr.futureverse.org/reference/with_progress.md)
  where the the “shiny” progression handler is by default added to the
  list of progression handlers used.
...
- [`with_progress()`](https://progressr.futureverse.org/reference/with_progress.md)
  gained arguments `enable` and `interval` as an alternative to setting
  corresponding options `progressr.*`.
...
- `with_progress(..., delay_conditions = "condition")`, introduced in
  **progressr** 0.1.0, would also capture conditions produced by
  ...
  e.g. [`progress::progress_bar()`](http://r-lib.github.io/progress/reference/progress_bar.md)
  output would not be displayed until the very end.
...
- [`with_progress()`](https://progressr.futureverse.org/reference/with_progress.md)
  now captures standard output and conditions and relay them at then
  end. This is done in order to avoid interweaving such output with the
  output produced by the progression handlers. This behavior can be
...
- Add [`print()`](https://rdrr.io/r/base/print.html) for `progression`
  conditions and `progressor` functions.
...
- Add support for `times = 1L` for progression handlers which when used
  will cause the progression to only be presented upon completion (=
  last step).

- The `shutdown` control_progression signaled by
  [`with_progress()`](https://progressr.futureverse.org/reference/with_progress.md)
  on exit now contains the `status` of the evaluation. If the evaluation
  was successful, then `status = "ok"`, otherwise `"incomplete"`.
...
- `with_progress(..., cleanup = TRUE)` requires a `withRestart()` such
  that also “shutdown” progressions can be muffled.
...
- The
  ...
  progression handler now displays the progression message.
...
- Add
  [`without_progress()`](https://progressr.futureverse.org/reference/with_progress.md) -
  which causes all `progression` conditions to be muffled and ignored.

### Bug Fixes

- Progressor functions could produce `progression` conditions that had
  the same identifiers and therefore would be considered duplicates such
  that progression handlers would ignore them.

- It was an error if a progression took a step big enough to skip more
  than the next milestone.

- Progression handlers now keep the internal `step` field within \[0,
  max_steps\] in case of a too big progression step is taken.
...
- Add a plyr-compatible “progress bar” named
  [`progress_progressr()`](https://progressr.futureverse.org/reference/progress_progressr.md).

- Add option `progressr.clear`.

- Visual progression handler will now always render the complete update
  state when `clear` is FALSE.

- Now progression handlers ignore a re-signaled `progression` condition
  if it has already been processed previously.

- Now each `progression` condition holds unique identifiers for the R
  session and for the progressor that produced the condition. It also
  contains an unique index per progressor that is incremented whenever a
  new `progression` condition is created.
...
- Add `intrusiveness` parameter that specifies how intrusive/disruptive
  a certain progress reporter is. For instance, an auditory reporter is
...
- Add
  [`progress_aggregator()`](https://progressr.futureverse.org/reference/progress_aggregator.md).
...
- Add progress update handlers based on
  ...
  `cat("\a")`,
  [`progress::progress_bar()`](http://r-lib.github.io/progress/reference/progress_bar.md),
  [`beepr::beep()`](https://rdrr.io/pkg/beepr/man/beep.html), and
  `notifier::notify()`.

- Add
  [`with_progress()`](https://progressr.futureverse.org/reference/with_progress.md).

- Add options `progressr.handlers` for settings default progress
  handlers.
...
- Add
  ...
  to create `progression` condition.
