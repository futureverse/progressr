...
circumstances. For example, it cannot be called within
...
The **[cli](https://cran.r-project.org/package=cli)** package can be
used to output colored text and messages in the terminal,
e.g. `message(cli::col_blue("hey"))`. However, **cli** disables such
coloring by default when there is an active progressor. The workaround
...
Setting the global progressr handler in `~/.Rprofile` does *not* work in
RStudio 2025.09:
...
(2025-03-20), which was most recently confirmed with Position 2025.09.0
...
typically happens just before handlers re-render the progress output.
This way you can use [`message()`](https://rdrr.io/r/base/message.html)
as usual, regardless whether progress is reported or not.

Currently, when using Positron (e.g. Positron 2025.09.0), any
[`message()`](https://rdrr.io/r/base/message.html) output adds an extra
newline before the *final* progress step is reported, e.g.
...
I do not fully understand the reason for this, but I hope we can get to
the bottom of it and fix it, either in **progressr** or in Positron,
e.g. [Bug \#9486](https://github.com/posit-dev/positron/issues/9486)
(2025-09-18).
...
*extra* newline at the end of every
...
reports via messages to render progress output very poorly or not at
all.
...
possible by the **[future](https://future.futureverse.org)** framework,
...
or the global progression handler, and relayed to the two progression
handlers ‘progress’ (a progress bar in the terminal) and ‘beepr’
(auditory) that the end user has chosen.*
